...
<?php
    ...
    <style>
.produits {
    background-color: #ffe4e1; /* Fond rosé clair */
    position: absolute;
    top: 60%;
    left:70%;
    margin-bottom: 40%;
...
.nom {
    ...
    text-align: center;
    color:black; /* Texte couleur rose foncé */
...
.desc label {
    ...
    display: block;
    margin-top: 10px;
    color: #d87093; /* Couleur rose pour les labels */
    font-family:'Times New Roman', Times, serif;
    font-size:25px;
}
...
button {
    width: 100%;
    padding: 10px;
    background-color: #f9bedb; /* Fond bouton rose vif */
    color: white;
    border: none;
    border-radius: 5px;
    cursor: pointer;
...
button:hover {
    background-color: #9c8c94; /* Fond bouton survolé rose plus clair */
}
.path{
    width:30%;
    margin-left:15%;
...
$requete="select * from produits where idp=$idp";
$resultat=$connexion->query($requete);
while ($ligne = $resultat->fetch_assoc()) {
?>
<form action="ajouterpanier.php?idp=<?php echo $ligne ["idp"]?>" method="post">
      <img class="path" src="<?php echo $ligne["image"] ?>" alt="" >
        <section class="produits">
            <div class="desc">
                <label for="" class="nom"><?php echo $ligne ["nompr"]?></label><br>
                <hr>
               

                <label for="" class="descriptionpr">Description</label><br>
                <p><?php echo $ligne["descriptionpr"] ?></p>

                <label for="prixpr" class="prixpr">Prix </label><br>
                <p><?php echo $ligne["prixpr"] ?> <sup>TND</sup> </p>

                <label for="number">Quantité</label><br>
                <input type="number" class="quantité" name="Quantity">
             <button type="submit">Ajouter au Panier<i class='bx bxs-basket'></i></button> 
